...
export class SegmentsComponent implements OnInit, AfterViewInit, OnDestroy {
   ...
   * frame closest to its own real timestamp, instead of every tile sharing
   * one static poster. Reuses the filmstrip sprite -- no new assets, no
   * extra network requests beyond the one (already-cached-after-first-load)
   * sprite image per row.
   */
